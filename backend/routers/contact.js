import contact from "../models/contact.js";
import User from "../models/user.js";
import { Router } from "express";
import authorize from "../middlewares/authorization.js";
import {sendMail} from "../services/mail.js"
// import requireAdmin from "../middlewares/admin.js";
import {sanitizeInput} from "../utilities/general.js"

const app = Router();


app.post("/support", async (req, res, next) => {
  try {
    for(const field in req.body){
      req.body[field] = sanitizeInput(req.body[field]);
      if(!req.body[field]) throw new req.AppError("invalid request data!")
    }
    
    const {email, subject, issue, message, priority} = req.body;
    
    if(!email.includes("@")) throw new req.AppError("invalid email!!")

    const contact = await Contact.create(req.body);

    sendMail({
      subject,
      template: "message",
      title: `${subject} - ${issue} - ${priority}`,
      description: `${message} <br> <br> <strong> sent by ${email}, contact id for more details ${contact._id}`
    }, req, sent => {
     return res.json({
        success: sent,
        message: sent ? "we received your email, we will review it shortly, fix the issues and notify you, thank you 🙏" : "opps something went wrong sending email, try again please"
      })
    })
  }catch(er){
    req.err = er;
    next()
  }
})


app.use(authorize);

// Get contacts with filters and pagination
app.get("/history",  async (req, res, next) => {
  try {
    const { _id } = req.user;
    const {
      page = 1,
      limit = 20,
      from,
      flag,
      seen,
      dateFrom,
      search
    } = req.query;
    
    for(const field in req.query){
      if(field !== "limit" && field !== "page") req.query[field] = sanitizeInput(req.query[field])
    }

    const query = { 
       $or: [{
         userId: _id
       }, {
         "for": "all"
       }]
    };
    
    const skip = (parseInt(page) - 1) * parseInt(limit);

    // Apply filters
    if (from) query.from = from;
    if (flag) query.flag = flag;
    if (seen !== undefined) query.seen = seen === 'true';
    if (dateFrom) query.createdAt = { gte: new Date(dateFrom) };
    
    
    // Search filter
    if (search) {
      const searchRegex = new RegExp(search, 'i');
      query.or = [
        { title: searchRegex },
        { description: searchRegex }
      ];
    }

    // Get contacts
    let contacts = await Contact.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit))
      .select("-__v -userId")
      .lean();
      
      // filter duplicate for all contact
     const contactCollectionKeys = [];
     contacts = contacts.filter(nt => {
       if(!contactCollectionKeys.includes(nt.collectionKey) && nt.for == "all" || nt.for == "single"){
         return nt
       }else{
         return false
       }
     })
     
       

    // Get total for pagination
    const total = await Contact.countDocuments(query);
    const totalPages = Math.ceil(total / parseInt(limit));
    const hasNext = page < totalPages;
    const hasPrev = page > 1;

    res.json({
      success: true,
      data: {
        contacts,
        pagination: {
          total,
          page: parseInt(page),
          limit: parseInt(limit),
          totalPages,
          hasNext,
          hasPrev
        }
      }
    });

  } catch (error) {
    req.err = error;
    next();
  }
});


// Mark contact as seen
app.put("/see",  async (req, res, next) => {
  try {
    const { _id } = req.user;
    const { contactId } = req.body;
    
    if(typeof contactId !== "string") throw new req.AppError("invalid contact id!!")
    
    if (!contactId) {
      return res.status(400).json({
        success: false,
        message: "contact ID is required"
      });
    }

    // Update contact
    const contact = await Contact.findOneAndUpdate(
      { 
        _id: contactId, 
        userId: _id 
      },
      { 
        seen: true 
      },
      { 
        new: true 
      }
    ).select("-__v -userId").lean();

    if (!contact) {
      return res.status(404).json({
        success: false,
        message: "contact not found"
      });
    }
    

    res.json({
      success: true,
      data: {
        contact
      }
    });
    
   return await User.findByIdAndUpdate({
     _id: req.user._id,
     contact: {
       $gte: 1
     }
   }, {
    $inc: {
    contact: -1
   }
    })

  } catch (error) {
    console.log(error)
    req.err = error;
    next();
  }
});

// Mark all contacts as seen
app.put("/see-all",  async (req, res, next) => {
  try {
    const { _id } = req.user;

    // Update all unseen contacts
    const result = await Contact.updateMany(
      { 
        userId: _id,
        seen: false 
      },
      { 
        seen: true 
      }
    );
    
    await User.findByIdAndUpdate(req.user._id,{
      $set: {
        contact: 0
      }
    })

    res.json({
      success: true,
      data: {
        modifiedCount: result.modifiedCount
      }
    });

  } catch (error) {
    req.err = error;
    next();
  }
});

app.delete("/old", async (req, res, next) => {
  try{
    const {user} = req;
    const { days = 30 } = req.query;
    const daysNum = parseInt(days);
  
    
    if (daysNum < 1) {
      return res.status(400).json({
        success: false,
        message: "Days must be at least 1"
      });
    }
    
    // Calculate cutoff date
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - daysNum);
    
    const query = {
      receivedAt: { $lt: cutoffDate }
    }
    
    if(user.role !== "admin" && user.role !== "super") query.userId = user._id;
    
    // Count contact to be deleted
    const count = await Contact.countDocuments(query);
    
    // Delete contact
    const result = await Contact.deleteMany(query);
    
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} contact older than ${daysNum} days`,
      data: {
        deletedCount: result.deletedCount,
        matchedCount: count,
        cutoffDate,
        days: daysNum
      }
    });
    
  }catch(er){
    req.err = er;
    next()
  }
})




export default app;